(() => {
    ...
    const keyShortcuts = {
        ...
        l: 'ln', g: 'log', r: 'sqrt',
        p: 'pi', e: 'e',
        d: 'deg', D: 'rad', // toggle degree/radian
        m: 'mr' // quick memory recall (hold as needed)
    };
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (k in keyShortcuts) {
            ...
            // special handling for deg/rad toggles and memory recall
            if (token === 'deg' || token === 'rad' || token === 'mr') { pressDataValue(token); return; }
        ...
        if (single && /[a-zA-Z]/.test(k)) {
            e.preventDefault();
            // if user typed a letter that doesn't match a shortcut, append it to the expression
            displayStr += k;
            evalStr += k;
            render();
            return;
